...
class ForecastFlow:
    ...
    def setup(self, project_name, bucket_name, is_timeout=False):
        ...
        # authenticate
        #if 'credential.json' not in os.listdir('/content'):
        #    get_credential_key = "wget https://storage.googleapis.com/ff-handson-seminar/credential.json"
        #    _exec_cmd(get_credential_key, verbose=False)
        #auth = "gcloud auth activate-service-account --key-file ./credential.json"
    ...
    def download(self):
        ...
        #    _exec_cmd(get_credential_key, verbose=False)
        #    _exec_cmd(auth, verbose=False)
        #dl = 'gsutil -m cp -r gs://handson-seminar/{} .'.format(self.project_name)
